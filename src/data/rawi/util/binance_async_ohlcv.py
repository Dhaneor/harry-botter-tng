...
MAX_WORKERS = 50  # max number of parallel requests to the Binance API
...
KLINES_LIMIT = 1500  # how many klines to request in one API call
BASE_DELAY = MAX_WORKERS / 25  # base delay for rate limiting (in seconds)

RATE_LIMIT_BARRIER = 6000  # this is the Binance API rate limit (see above)
...
RAW_REQUEST_LIMIT = 61000  # requests per 5 minutes (see above)
...
class CircuitBreaker:
    ...
    def __init__(self, max_failures=10, reset_time=60):
        self.failures = 0
    ...
    def seconds_to_reset(self) -> float:
        if self.is_open():
            delay = (
                ...
                + timedelta(seconds=self.reset_time)
                ) - datetime.now()
