...
def is_below(arr: np.ndarray, x: np.float64 | np.int_) -> npt.NDArray[np.bool_]:
...
def is_equal(
    arr: np.ndarray, x: npt.ArrayLike | np.float64 | np.int_ | np.bool_
) -> np.ndarray[np.bool_]:
    """Check whether values in array are below a certain value.
...
def crossed_above(arr1: np.ndarray, arr2: np.ndarray | float | int) -> np.ndarray:
...
def crossed_below(arr1: np.ndarray, arr2: np.ndarray | float | int) -> np.ndarray:
    """Checks whether one time series values crossed above value.
