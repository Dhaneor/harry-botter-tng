...
Provides a clqss to observe OHLCV data from a cryptocurrency exchange.

Other objects that are interested in this data can register to this observer
and will be notified for every trading interval they are subscribed to.
...
from src.exchange.kucoin_ import Public as ExchangePublic
from .ohlcv_validator import OhlcvValidator
from exchange.util.cryptowatch import CryptoWatch

from broker.config import CREDENTIALS  # noqa: F401
...
class TimeDifferenceWatcher(Thread):
    ...
    def _initialize_deque(self):
        for _ in range(2):
            try:
                self._handle_query_result(self.exchange.get_server_time())
            except Exception:
    ...
    def _extract_server_time_from_query_result(
    ...
    def run(self):
        ...
        while True:
            ...
            if self.expired_intervals:
                logger.debug(f"callback triggered at {datetime.datetime.utcnow()}")
...
class OhlcvObserver:
    ...
    def register_subscriber(
        self, id: str, symbols: Iterable[str], interval: str, callback: Callable
    ):
        ...
        try:
            if not interval not in self.subscribers.keys():
    ...
    def _download_ohlcv(self, requests: Iterable[Tuple[str, str]]) -> List[dict]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for r in requests:
                futures.append(
                    executor.submit(
                        self._download_ohlcv_for_one_symbol, symbol=r[0], interval=r[1]
                    )
                )
    ...
    def _is_valid_result(self, ohlcv: pd.DataFrame, interval: str, start: int) -> bool:
        ...
        if not isinstance(ohlcv, pd.DataFrame):
            logger.error("downloaded data came not back as dataframe")
            return False

        if ohlcv.empty:
            logger.error("we only got an empty dataframe")
            return False
    ...
    def _download_from_alternative_data_source(
        ...
        logger.debug(f"downloaded data from crpytowatch - rows: ({len(data)})")
    ...
    def _notify_subscribers(self, interval: str, data: List[dict]):
        ...
        if interval in self.subscribers.keys():
            ...
            for recipient in self.subscribers[interval]:
                ...
                try:
                    executor.submit(
                        recipient.callback, {"id": recipient.id, "data": message}
                    )
