...
class Merlin:
    ...
    def get_tickers(self):
        ...
        for i in range(1, 16): df.iloc[:, i] = df.iloc[:, i].astype(float)
    ...
    def _sort_by_price_change(self):
        self.tickers.sort_values(by=['priceChangePercent'], ascending=False, inplace=True)
    ...
    def _add_atr_percent(self):
        ...
        for item in self.klines:
            ...
            self.tickers.loc[self.tickers['symbol'] == symbol, 'atr.perc'] = df['ATR percent'].iloc[-1]

    def _add_indicators_to_klines(self):

        for idx, kline in enumerate(self.klines):
            self.klines[idx] = (self.indicators.average_true_range(df=kline[0], period=720, method='ewm'),
                                self.klines[idx][1],
                                self.klines[idx][2])
