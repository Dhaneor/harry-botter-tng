...
class IExitOrderStrategy(ABC):
    ...
    @abstractmethod
    def get_trigger_prices_np(self, open_: np.ndarray, high: np.ndarray,
...
class IStopLossStrategy(IExitOrderStrategy):
    ...
    def get_stop_loss_prices(self, df: pd.DataFrame,
                        long_or_short: str) -> Tuple[Tuple[price, fraction]]:
        ...
        return tuple(
            tuple(
                (df[col].iat[-1], fraction)
            )
            for col in df.columns if str_ in col
        )
...
class AtrStopLossStrategy(IStopLossStrategy):
    ...
    def add_stop_loss_prices_to_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        atr_series = atr(
            ...
            df['low'].to_numpy(), self.atr_lookback
        )
...
class ITakeProfitStrategy(IExitOrderStrategy):
    ...
    def get_take_profit_prices(self, df: pd.DataFrame,
                               long_or_short: str
                               ) -> Tuple[Tuple[price, fraction]]:
        """Calculates stop loss price(s) and their fractions.
...
class AtrTakeProfitStrategy(ITakeProfitStrategy):
    ...
    def get_take_profit_prices(self, df: pd.DataFrame, long_or_short: str
                               ) -> Tuple[Tuple[price, fraction]]:
...
valid_sl_strategies = {
    'pct': PercentStopLossStrategy,
    'atr': AtrStopLossStrategy
}

valid_tp_strategies = {
    'pct': PercentTakeProfitStrategy,
    'atr': AtrTakeProfitStrategy
}
