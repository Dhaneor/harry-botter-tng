...
class IAnalyst(ABC):
    ...
    def draw_chart(self, df:pd.DataFrame, subplots: Optional[dict]=None,
                   color_scheme:str='day', with_market_state: bool=False):
...
class RsiAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.comment: str = f'oversold = {self.oversold}, overbought = {self.overbought}'
    ...
    def get_signal(self, data: dict, mode: int=0, lookback:int=14):
        ...
        rsi = data[col] = self.indicators.rsi_ta_lib(
            data=data['c'], period=lookback
        )
...
class ConnorsRsiAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.comment: str = f'oversold = {self.oversold}, ' \
    ...
    def get_signal(self, df:pd.DataFrame, on_what:str='close',
                   rsi_lookback:int=2, streak_lookback:int=3,
                   smoothing:int=1):
        ...
        df[col] = self.indicators.connors_rsi(
            data=df[on_what], rsi_lookback=rsi_lookback,
...
class DynamicRateOfChangeAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.plot_params = {'label' : 'TD Rate-Of-Change',
                            'columns' : ['droc.close'],
                            'horizontal lines' : [],
                            'channel' : [],
                            'fill': True,
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df:pd.DataFrame, on_what:str='close',
                   lookback:int=14, smoothing:int=1) -> pd.DataFrame:

        df = self.indicators.dynamic_rate_of_change(
            df=df, on_what=on_what,
...
class StochRsiAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.plot_params = {'label' : 'Stochastic RSI',
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df:pd.DataFrame, method='crossover', period_rsi:int=14,
                   period=14, k_period=5, d_period=3):

        df = self.indicators.stoch_rsi(
            df=df, period_rsi=period_rsi, period=period,
        ...
        # ---------------------------------------------------------------------
            ...
            prev_k_line, prev_d_line = df['stoch.rsi.k'].shift(), df['stoch.rsi.d'].shift()
            o_bought, o_sold = self.overbought, self.oversold

            conditions = [
                (k_line > d_line) & (prev_k_line <= prev_d_line) & (prev_k_line <= o_sold),
                (k_line < d_line) & (prev_k_line >= prev_d_line) & (prev_k_line >= o_bought)
            ]
...
class StochasticAnalyst(IAnalyst):
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df: pd.DataFrame, oversold: int=30,
...
class TrendAnalyst(IAnalyst):
    ...
    def get_signal(self, df:pd.DataFrame):
        ...
        # ---------------------------------------------------------------------
        df[col_slp_prc] = round(
            (df[col_price] / df[col_price].shift() - 1) * 100, 4
        )
...
class BigTrendAnalyst(IAnalyst):
    ...
    def get_signal(self, df: pd.DataFrame, period: int=20, factor: float=0.001,
                   fast_exit: bool=False) -> pd.DataFrame:
...
class MovingAverageAnalyst(IAnalyst):
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df:pd.DataFrame, period:int=63, type='ewma') -> pd.DataFrame:
        if type == 'sma':
...
class MomentumAnalyst(IAnalyst):
    ...
    def get_signal(self, df:pd.DataFrame) -> pd.DataFrame:
        ...
        if not col_short in df.columns:
            df = self.indicators.sma(df=df, on_what=col_mom,
                                     period=self.short_sma, col_prefix='mom.')

        if not col_long in df.columns:
            df = self.indicators.sma(df=df, on_what=col_mom,
                                     period=self.long_sma, col_prefix='mom.')

        # ---------------------------------------------------------------------
        mom, short_ma, long_ma = df['mom'], df[col_short], df[col_long]
        prev_mom, prev_short_ma, prev_long_ma = mom.shift(), short_ma.shift(), long_ma.shift()
        ...
        conditions = [(short_ma > long_ma) & (prev_short_ma <= prev_long_ma),
...
class AverageDirectionalIndexAnalyst(IAnalyst):
    ...
    # expects a dataframe with OHLC data and calculates the RSI signal
    # for the last row, if no index is given
    def get_signal(self, df:pd.DataFrame, lookback:int=14,
                   threshhold:int=25) -> pd.DataFrame:
        ...
        # ---------------------------------------------------------------------
        conditions = [(adx > threshhold) & (plus_di > minus_di),\
...
class ChoppinessIndexAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.plot_params = {'label' : 'Choppiness Index',
                            'columns' : ['ci'],
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df:pd.DataFrame, lookback:int=14,
                   atr_lookback:int=14) -> pd.DataFrame:
...
class NoiseAnalyst(IAnalyst):
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df: pd.DataFrame, period: int=14,
                   smoothing: int=14) -> pd.DataFrame:
...
class TrendyAnalyst(IAnalyst):
    ...
    def get_signal(self, df: pd.DataFrame, lookback: int=14,
                   smoothing: int=1, threshhold: float=2) -> pd.DataFrame:
...
class WickAndBodyAnalyst(IAnalyst):
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df:pd.DataFrame, factor:float=2,
                   confirmation:bool=True) -> pd.DataFrame:
        ...
        if confirmation:
            upper_wick = abs(df['high'].shift() \
                - df[["open", "close"]].shift().max(axis=1))
            lower_wick = abs(df[["open", "close"]].shift().min(axis=1)\
                - df['low']).shift()
            ...
            conditions = [(upper_wick > body) & (upper_wick > lower_wick)\
                            & (direction == -1), \
...
class BreakoutAnalyst(IAnalyst):
    ...
    def _find_breakouts_with_variable_lookback(self, df: pd.DataFrame,
    ...
    def _get_variable_lookback(self, df, base_lookback) -> pd.Series:
        short_atr = i.average_true_range(
            open_=df['open'], high_=df['high'], low_=df['low'],
            close_=df['close'], period=7
        )
        long_atr = i.average_true_range(
            open_=df['open'], high_=df['high'], low_=df['low'],
            close_=df['close'], period=60
        )
...
class TDClopwinPatternAnalyst(IAnalyst):
    ...
    def get_signal(self, df:pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[
            (df[self.column_name] == -1) & (df['close'] < ewm),
            self.column_name
            ] = 0
        df.loc[
            (df[self.column_name] == 1) & (df['close'] > ewm),
...
class DisparityAnalyst(IAnalyst):
    ...
    def __init__(self):
        ...
        self.plot_params = {'label' : 'Disparity',
                            'columns' : ['disp'],
    ...
    # -------------------------------------------------------------------------
    def get_signal(self, df: pd.DataFrame,
                   lookback_periods: List[int]=[21, 200, 500]) -> pd.DataFrame:
