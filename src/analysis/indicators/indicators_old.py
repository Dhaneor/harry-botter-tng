    ...
    def average_true_range(self, 
        ...
        if df is None:
            ...
            if any(arg is None for arg in (open_, high_, low_, close_)):
                raise ValueError(f"Please provide value series for all of: "
...
# -----------------------------------------------------------------------------
    ...
    def sma(self, df: pd.DataFrame, period:int, on_what:str='close', 
        ...
        if on_what in df.columns:
            df[column] = df[on_what].rolling(period)\
                                    .mean()\
                                    .round(self._qp) \
                                    .astype(float)
            return df
    ...
    def ewma(self, df: pd.DataFrame, period: int, on_what: str='close',
             ) -> pd.DataFrame:
...
# -----------------------------------------------------------------------------
    ...
    def rsi_ta_lib(self, data: TimeSeries, period: int=14
                   ) -> npt.NDArray[np.float64]:
        
        return ta.RSI(data, timeperiod=period)

    def connors_rsi(self, data: pd.Series, rsi_lookback:int=2, streak_lookback:int=3) -> pd.Series:
        ...
        rank = delta\
            .rolling(100)\
            .apply(lambda x: pd.Series(x).rank(pct=False).iloc[-1])
        
        connors_rsi = (rsi + rsi_streak + rank) / 3
        
        return connors_rsi
        
    def stochastic(self, df: pd.DataFrame, on_what:str='close', period:int=5,
    ...
    def stoch_rsi(self, df: pd.DataFrame, period_rsi: int=14, period: int=5,
                  k_period: int=3, d_period: int=3) -> pd.DataFrame:
        ...
        return self.stochastic(
            df=df, on_what=rsi_column, period=period, 
            k_period=k_period, d_period=d_period
        )

    def momentum(self, df: pd.DataFrame, lookback: int=14, on_what: str='close'
                 ) -> pd.DataFrame:
        ...
        df['mom'] = (((df[on_what] / df[on_what].shift(lookback)) * 100) -100)\
    ...
    def dynamic_rate_of_change(self, df: pd.DataFrame, on_what='close', 
                              lookback: int=14, smoothing: int=1, 
        ...
        if smoothing > 1:
            df[col_name] = df[col_name].rolling(smoothing)\
                                      .mean()\
                                      .round(4)\
                                      .astype(float) 
    ...
    def _rate_of_change(self, df:pd.DataFrame, lookback:int=1,
                              on_what='close') -> pd.Series:
    ...
    def fibonacci_trend(self, df: pd.DataFrame, max_no_of_periods: int=100, 
                        smoothing: int=9) -> pd.DataFrame:
        ...
        df['f_trend'] = (
            _df.pct_change(axis=1)
                .ewm(span=smoothing).mean()
                .sum(axis=1) * -1
        )
    ...
    @execution_time
    def fibonacci_trend_nb(self, df: pd.DataFrame, max_no_of_periods: int=100, 
                           smoothing: int=9) -> pd.DataFrame:
    ...
    def big_trend(self, close: pd.Series, period: int, factor: float=0.001
    ...
    # -------------------------------------------------------------------------
    def bollinger(self, data: TimeSeries, period: int=20, multiplier: float=2, 
                  ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def keltner_channel(self, df: pd.DataFrame, period: int=20, 
                        multiplier: float=2, atr_lookback: int=20
                        ) -> pd.DataFrame:
    ...
    def keltner_talib(
        ...
        low: TimeSeries, 
        close: TimeSeries, 
        period: int=20, atr_multiplier: float=2,
    ...
    def adx(self, df:pd.DataFrame, lookback:int=7, atr_lookback:int=14
            ) -> pd.DataFrame:
        ...
        d.loc[
            (d.high_diff > d.low_diff) & (d.high_diff > 0), 'di_up'
        ] = d.high_diff
        
        d.loc[
            (d.high_diff < d.low_diff) & (d.low_diff < 0), 'di_down'
        ] = d.high_diff
...
#                                   MARKET STATE                              #
# -----------------------------------------------------------------------------
    def choppiness_index(self, df: pd.DataFrame, lookback:int=14, 
        ...
        df['ci'] = 100 * np.log10((atr.rolling(lookback).sum()) \
                    / (highest_high - lowest_low)) \
                        / np.log10(lookback)
        
        return df

    def disparity_index(self, df: pd.DataFrame, column_a: pd.Series, 
                        column_b: pd.Series, column_c: pd.Series,
                        as_percentiles: bool=False) -> pd.Series:

        # TODO: this is still incomplete?!
        return column_b / column_c - 1

    @execution_time
    def noise_index(self, data: pd.Series, period: int=3, 
                    smoothing: int=1) -> pd.Series:
