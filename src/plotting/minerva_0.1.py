...
class Minerva:
    ...
    def draw_detailed_chart(self, df: pd.DataFrame):
        ...
        fig, axes = plt.subplots(
            ...
            figsize=(12, 5),
            sharex=True,
            gridspec_kw={"width_ratios": [1], "height_ratios": [9, 1, 1, 3, 1, 2]},
        )
        fig.set_dpi(160)
        ...
        if "cool.off" in df.columns:
            for x_ in range(len(df)):
                if df.iloc[x_]["cool.off"] == True:
                    ax1.axvline(
                        x=x_, color=color_sell, alpha=0.2, linewidth=lw, linestyle="-"
                    )
            ...
            if "s.stc" in df.columns:
                for idx in df.index:
                    ...
                    if df.loc[idx, "s.stc"] == 1:
                        ax2.axvline(
                            x_, color=color_buy, linewidth=0.2, alpha=signal_alpha
                        )
                    if df.loc[idx, "s.stc"] == -1:
                        ax2.axvline(
                            x_, color=color_sell, linewidth=0.2, alpha=signal_alpha
                        )
        ...
        if "mom" in df.columns:
            ...
            if "s.mom" in df.columns:
                for idx in df.index:
                    x_ = idx - start_index
                    if df.loc[idx, "s.mom"] == 1:
                        ax4.axvline(
                            x_, color=color_buy, alpha=signal_alpha, linewidth=0.2
                        )
                    if df.loc[idx, "s.mom"] == -1:
                        ax4.axvline(
                            x_, color=color_sell, alpha=signal_alpha, linewidth=0.2
                        )
            ...
            ax4.axhline(
                y=threshhold_high, color="green", linestyle="dotted", linewidth=0.1
            )
            ax4.axhline(
                y=threshhold_high * -1, color="green", linestyle="dotted", linewidth=0.1
            )
...
class OracleChart(Minerva):
    ...
    # -------------------------------------------------------------------------
    def draw_signal_chart(
        self, cols_subplot_1, cols_subplot_2, slope_cols, signal_cols
    ):
        ...
        fig, axes = plt.subplots(
            ...
            sharex=True,
            figsize=(12, 5.5),
            gridspec_kw={"width_ratios": [1], "height_ratios": [6, 1, 1, 1, 1]},
        )
        ...
        for idx in df.index:
            ...
            try:
                if df.loc[idx, "s.all"] > 0:
                    ax_all.axvline(
                        x, color=color, alpha=alpha, linewidth=lw, linestyle="-"
                    )
            except:
                pass
        ...
        ax_mom.axhline(
            y=0, color="grey", linestyle="dotted", alpha=alpha, linewidth=0.5
        )
        ...
        threshhold_high = 9
        ax_mom.axhline(
            y=threshhold_low, color="green", linestyle="dotted", linewidth=0.5
        )
        ax_mom.axhline(
            y=threshhold_high, color="green", linestyle="dotted", linewidth=0.5
        )
        ...
        ax_rsi.axhline(
            y=0, color="green", linestyle="dotted", alpha=alpha, linewidth=0.5
        )
