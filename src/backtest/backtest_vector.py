...
class Backtest:
    ...
    def reevaluate_position_change(self, df) -> pd.DataFrame:
        ...
        df.loc[
            ...
            | (df["p.type"] == "SHORT")
            & (df["p.type"].shift() == "LONG")
            & (df["sl.trig"].shift() == False),
            "p.change",
        ] = 2

        df.loc[(df["p.type"] == "LONG") & (df["sl.trig"] == True), "p.change"] = -1

        df.loc[(df["p.type"] == "SHORT") & (df["sl.trig"] == True), "p.change"] = -1
    ...
    def find_buy_and_sell_prices(self, df) -> pd.DataFrame:
        ...
        df.loc[(df["buy"] == "•"), "buy.price"] = df["open"]  # & (df['s.all'] == 1),

        df.loc[(df["sell"] == "•") & (df["s.all"] == -1), "sell.price"] = df["open"]

        # set buy/sell price for SL triggered positions
        df.loc[(df["p.type"] == "LONG") & (df["sl.trig"]), "sell.price"] = df[
            "sl.current"
        ]

        df.loc[(df["p.type"] == "SHORT") & (df["sl.trig"]), "buy.price"] = df[
            "sl.current"
        ]
    ...
    def find_buy_and_sell_amounts(self, df) -> pd.DataFrame:
        ...
        # position opened
        df.loc[(df["buy"] == "•") & (df["position"].shift() != 1), "buy.amount"] = (
            df["b.value"] / df["close"]
        ) * df["p.size"]

        df.loc[(df["sell"] == "•") & (df["position"].shift() != -1), "sell.amount"] = (
            df["b.value"] / df["close"]
        ) * df["p.size"]
        ...
        df.loc[(df["sell"] == "•") & (df["position"].shift() == 0), "p.size"] = df[
            "sell.amount"
        ]
        ...
        # position closed
        df.loc[(df["sell"] == "•") & (df["position"] == 1), "sell.amount"] = df[
            "p.size"
        ]

        df.loc[(df["buy"] == "•") & (df["position"] == -1), "buy.amount"] = df["p.size"]
    ...
    def add_leverage(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[
            (df["p.type"] == "LONG") & ~(df["p.type"].shift() == "LONG"), "leverage"
        ] = df["leverage.max"]

        df.loc[
            (df["p.type"] == "SHORT") & ~(df["p.type"].shift() == "SHORT"), "leverage"
        ] = df["leverage.max"]
    ...
    def add_returns(self, df):
        ...
        df["s.returns"] = 0
        df.loc[df["p.actv"] == "•", "s.returns"] = (
            df["returns.pct"] * df["p.size"] * df["position"]
        )

        df.loc[(df["sell"] == "•") & (df["sl.trig"] == True), "s.returns"] = (
            df["sl.current"] / df["close"].shift() - 1
        ) * df["p.size"]

        df.loc[(df["buy"] == "•") & (df["sl.trig"] == True), "s.returns"] = (
    ...
    def __process_buy(self, df, idx):
        ...
        if df.at[idx, "p.type"] == "LONG":
            ...
            df.at[idx, "b.quote"] = (
                df.at[idx, "b.quote"] - df.at[idx, "b.base"] * df.at[idx, "buy.price"]
            )
    ...
    def _calculate_balance_value(self, df: pd.DataFrame):
        ...
        for idx in range(df.first_valid_index() + 1, df.last_valid_index() + 1):
            df.at[idx, "b.value"] = (
                df.at[idx - 1, "b.value"]
                + df.at[idx - 1, "b.value"] * df.at[idx, "s.returns"]
            )  # * df.at[idx, 'p.size']
    ...
    def _calculate_capital(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[(df["sell"] == "•"), "capital"] = df["b.value"]
        df.loc[(df["buy"] == "•"), "capital"] = df["b.value"]
    ...
    def _calculate_hodl_pnl(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["hodl.value"] = (hodl_qty * df["close"]).round(
    ...
    def _calculate_sharpe_ratio(
        self, df: pd.DataFrame, column: str = "b.value", trading_days: int = 365
    ) -> float:
    ...
    def show_overview(self, df):
        ...
        print(df.loc[self.moneta.start_index : self.moneta.end_index, include_columns])
