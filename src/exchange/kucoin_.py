    ...
    def inner(*args, **kwargs):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f'{func.__name__} took {(time() - st)*1000:.0f} ms'
                )
    return inner
...
class Public(IExchangePublic, OhlcvDownloadPrepper,
    ...
    def get_symbols(self, quote_asset:Union[str, None]=None) -> Tuple[dict]:
        ...
        if symbols:
            ...
            return tuple(
                self._standardize_symbol_information(s) for s in symbols
            )
        else:
            return tuple()
    ...
    def get_symbol(self, symbol: str) -> Union[dict, None]:
        return next(
            filter(lambda x: x['symbol'] == symbol, self.get_symbols()),
            None
        )
...
class Account(KucoinResponseFormatter):
    ...
    @wrap_call
    def get_deposits(self, currency=None, status=None, start=None, end=None,
...
class CrossMargin(IExchangeTrading, IExchangeMargin, KucoinResponseFormatter):
    ...
    @wrap_call
    def get_fees(self, symbols: Union[list[str], str]
    ...
    def get_orders(self, symbol: Optional[str]=None, side: Optional[str]=None,
        ...
        def _add_to_final_result(res):
            if 'symbol' in res:
                ...
                raise Exception(
                    f'something went wrong with the order downloads {res}'
                )
    ...
    def get_active_orders(self, symbol: Optional[str]=None,
        ...
        res =  self.trade_client.get_order_list(
            symbol=symbol, status='active', tradeType='MARGIN_TRADE',
        )
    ...
    def get_active_stop_orders(self, symbol: Union[str, None]=None
        ...
        if symbol:
            return tuple(
                self._standardize_stop_order_response(o) for o in res['items']\
                    if o['symbol'] == symbol
            )
    ...
    def _get_multiple_orders(self,
        ...
        if any(arg is not None for arg in (start, end)):
            ...
            orders = kod.get_orders(
                ...
                start=start, end=end # type:ignore
            )
        ...
        # Kucoin considers to be recent orders
            ...
            while True:
                ...
                res = self.trade_client.get_order_list(
                    symbol=symbol, status=status, tradeType='MARGIN_TRADE',
