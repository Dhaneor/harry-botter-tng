...
class OhlcvDownloadPrepper:
    ...
    def _prepare_request(
        ...
        if not interval in self.INTERVALS.keys():
        ...
        number_of_chunks = self._get_number_of_chunks(
            start_ts, end_ts, interval
        )
        
        # get the chunks (periods for every download step)
        chunks = self._get_chunk_periods(start_ts=start_ts, end_ts=end_ts,
                                         interval=interval) 
    ...
    def _get_chunk_periods(self, start_ts:int, end_ts:int, interval:str):
        limit = self.limit
        steps = []
        ms_per_interval = interval_to_milliseconds(interval) / 1000
        step_start = start_ts
        
        while not step_start >= end_ts:
            step_end = int(step_start + limit * ms_per_interval - 1)
    ...
    def _get_timestamps_for_period(self, symbol:str, start: Union[str, int], 
                                   end: Union[str, int], interval: str) -> tuple:

        start_ts, end_ts = get_start_and_end_timestamp(
            start=start, end=end, interval=interval, 
            unit='seconds', verbose=False
        )
        ...
        if number_of_epochs > 5:
            ...
            logger.debug(f'Listing date: {ld_human} {type(listing_date)}, start: {start_ts_human}')
            
            earliest_valid_ts = max(
                start_ts,
                listing_date,
                utc_to_unix('January 01, 2017 00:00:00')
            )
         
        # convert timestamp to seconds if it was given in seconds   
         
        start_ts = int(start_ts / 1000) if len(str(start_ts)) > 10 else start_ts 
        end_ts = int(end_ts / 1000) if len(str(end_ts)) > 10 else end_ts
        
        logger.debug(f'Start timestamp: {start=} -> {start_ts}')
        logger.debug(f'End timestamp: {end=} -> {end_ts}')
    ...
    def _get_listing_date(self, symbol: str, interval: str='1d') -> int:
        logger.debug(
            f'getting listing date for symbol: {symbol} ({interval}) on {self.name}'
        )
        ...
        res = self._get_earliest_valid_timestamp(
