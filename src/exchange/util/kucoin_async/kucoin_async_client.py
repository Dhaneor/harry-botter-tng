...
class AsyncKucoinBaseRestApi(object):
    ...
    async def _request(self, method, uri, timeout=5, auth=True, params=None):
        ...
        headers["User-Agent"] = "kucoin-python-sdk/v1.0.11"
        ...
        if method == 'GET':
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.post(url=url, data=data_json) as r:
                    response = await r.json()
        elif method == 'DELETE':
            async with aiohttp.ClientSession(headers=headers) as session:
    ...
    def check_response_data(response_data):
        ...
        code, msg = response_data['code'], response_data['msg']
