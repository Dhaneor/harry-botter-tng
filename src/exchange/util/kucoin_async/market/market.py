...
class MarketData(AsyncKucoinBaseRestApi):
    ...
    async def get_currency_detail_v2(self, currency, chain=None):
        """
        ...
        if chain:
            params['chain'] = chain
        return await self._request('GET', '/api/v2/currencies/{currency}'.format(currency=currency), params=params)
