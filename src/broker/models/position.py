...
class Position(ABC):
    ...
    def symbols_for_asset(self):
        return tuple(
            sym['symbol'] for sym in self.broker.symbols \
    ...
    def last_price(self, value: Union[float, str]):
        try:
            ...
            self.logger.error(
                f'cannot set last price from {value} {type(value)}'\
                    f'for {self.asset}'
            )
        ...
        try:
            if self.symbol and msg['symbol'] == self.symbol.name:
                ...
                if msg['type'] == 'open':
                    ...
                    self.logger.debug(
                        f'added order {order_id} to active stop orders'
                    )
                    return
                if msg['type'] == 'cancel':
                ...
                raise ValueError(
                    f'stop order update for wrong symbol')
        except Exception as e:
            self.logger.error(
                f'failed to handle stop order update: {msg} -> {e}'
            )
            raise ValueError(e)
...
class PositionDetective(ABC):
    ...
    @property
    def orders(self):
        return [o for o in self.broker.orders \
            if o.symbol in self.position.symbols_for_asset
            ]
    
    @property
    def active_stop_orders(self):
        return [o for o in self.broker.orders \
            if all(arg for arg in (
    ...
    def _add_entry_orders(self):
        self.position.entry_orders = [
            o for o in self.orders if all(
                arg for arg in (
                    o.side == self.position.ENTRY_SIDE,
                    o.status == 'FILLED'
                )
            )
        ]
    ...
    def _add_exit_orders(self):
        self.position.exit_orders = [
            o for o in self.orders if all(
                arg for arg in (
                    o.side == self.position.EXIT_SIDE,
                    o.status == 'FILLED'
                )
            )
        ]
...
class PositionFactory:
    ...
    def build_positions(self, account: List[dict], 
                        quote_asset: str) -> Tuple[Position]:
    ...
    def build_position_from_balance(self, balance: Balance) -> Position:
        ...
        position.logger = logging.getLogger(
            f'main.Position.{position.asset}'
        )
        ...
        if position.asset == position.quote_asset:
            ...
            try:   
                ...
                position.symbol = None
                print(f'failed to build position from balance for '\
                    f'{balance.asset}: {e}')
        ...
        try:
            ...
            self.logger.error(
                f'failed to add stop orders to position {position} - {e}'
            )
    ...
    def _add_active_stop_orders_to_position(self, position: Position):
        if not self.stop_order_download_done:
            ...
            if self.all_active_stop_orders:
                self.logger.debug(
                    f'got {len(self.all_active_stop_orders)} orders'
                )
