...
def unix_to_utc(unix: int) -> str:
    ...
    try: 
        humanDate = datetime.utcfromtimestamp(
            int(unix)).strftime('%Y-%m-%d %H:%M:%S')
...
def get_start_and_end_timestamp(start:Union[int, str], 
                                end:Union[int, str],
                                interval:str, 
...
def time_difference_in_ms(start: float, end: Optional[float]=None, 
                          precision : int=2):
...
def test_date_to_milliseconds():
    ...
    dates = ['February 01, 2022 00:00:00', '2022-03-01 00:00:00', 
             'UTC now', 'now UTC',
             '2 weeks ago' ,'30 days ago']
...
def test_start_end_timestamps():
    periods = [('2021-01-01 00:00:00', '2022-01-01 00:00:00'), 
               (-1000, 'now UTC'),
               (-2000, None),
               (0, 'now UTC'),
